...
const DECK_POPULATION = { path: 'owner', select: 'username role' };
...
const buildStandardDeckFilter = (user) => (isAdmin(user) ? { isOfficial: false } : { owner: user._id, isOfficial: false });

const buildOfficialDeckFilter = () => ({ isOfficial: true, level: 'beginner' });

const ownsRecord = (record, user) => isAdmin(user) || String(record.owner) === String(user._id);
...
exports.createDeck = async (req, res) => {
  try {
    ...
    if (payload.isOfficial) {
      return res.status(403).json({ message: 'Official beginner decks must be created through the admin-only official deck flow.' });
    }
...
exports.createOfficialBeginnerDeck = async (req, res) => {
  try {
    if (!isAdmin(req.user)) {
      return res.status(403).json({ message: 'Only admins can create official beginner decks.' });
    }
...
exports.updateDeck = async (req, res) => {
  try {
    ...
    if (deck.isOfficial) {
      return res.status(403).json({ message: 'Official beginner decks must be updated through the admin-only official deck flow.' });
    }
...
exports.updateOfficialBeginnerDeck = async (req, res) => {
  try {
    if (!isAdmin(req.user)) {
      return res.status(403).json({ message: 'Only admins can update official beginner decks.' });
    }
...
exports.deleteDeck = async (req, res) => {
  try {
    ...
    if (deck.isOfficial) {
      return res.status(403).json({ message: 'Official beginner decks must be deleted through the admin-only official deck flow.' });
    }
...
exports.deleteOfficialBeginnerDeck = async (req, res) => {
  try {
    if (!isAdmin(req.user)) {
      return res.status(403).json({ message: 'Only admins can delete official beginner decks.' });
    }
...
exports.importDeckToOfficialBeginnerDeck = async (req, res) => {
  try {
    if (!isAdmin(req.user)) {
      return res.status(403).json({ message: 'Only admins can import decks into Official Beginner Decks.' });
    }
