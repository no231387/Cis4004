...
function DecksPage() {
  ...
  return (
    <section>
      <div className="card">
        <h2>Decks</h2>
        <p>{isAdmin ? 'Admins can view and manage all normal decks.' : 'Create and manage decks for your own study collections.'}</p>
      </div>
      ...
      <div className="list-grid">
        {decks.map((deck) => (
          <article key={deck._id} className="card">
            ...
            {isAdmin && deck.owner && (
              <p>
                <strong>Owner:</strong> {deck.owner.username || deck.owner}
              </p>
            )}
            <div className="action-row">
              ...
              {isAdmin && String(deck.owner?._id || deck.owner) === String(user?._id) && (
